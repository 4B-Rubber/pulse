...
重命名框隐藏了原生光标（自绘位图里画自己的），输入法因此可能问不到光标归属。已经踩过两条路径，都由宿主编辑框兜住：

- 提交组词时用 `WM_SETTEXT` 重写整段文本：EDIT 会把光标放回 0。宿主在文本没变时恢复原选区，文本被改写时把光标放在“变化段之后”。
- 提交组词时用 `WM_CHAR` 逐字上屏：TSF 桥随后补一条 `EM_SETSEL(0,0)`。宿主在“刚提交完字符”的两秒窗口内丢弃这条越界请求；鼠标点击、方向键、焦点变化立即解除该保护；普通字符不解除（输入法会把这些按键透传给编辑框，把它当导航键处理会让快速连打时保护失效）。
- 诊断：`%LOCALAPPDATA%\Pulse\ime_caret.log` 记录 `armed-first` / `drop` / `keep`；`PULSE_EDIT_TRACE=<file>` 记录宿主编辑框收到的每条消息（文本、选区、组词串、保护状态）。
- 覆盖用例：`PULSE_SELFTEST_CASE=rename-editor` 的 5 条断言——冗余 WM_SETTEXT、改写 WM_SETTEXT、提交后越界重置、透传按键、导航键。
